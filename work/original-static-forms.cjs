...
function renderOriginalStaticForm(slug,prefix='../'){let markup=forms[slug]||'';
 if(slug==='fivecred-landing-page'&&!markup.includes('form-unavailable')){
  ...
  markup=markup.replace(/(<form[^>]*>)/,'$1<p class="form-unavailable" role="status">O envio deste formulário está temporariamente indisponível. Você pode falar com a equipe pelos canais de atendimento da página.</p>');
 }
 return markup;}
